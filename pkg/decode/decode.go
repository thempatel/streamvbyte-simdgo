...
// Prev: 80
func Get8uint32DiffScalar(in []byte, out []uint32, ctrl uint16, prev uint32) {
	lower := uint8(ctrl&0xff)
	upper := uint8(ctrl>>8)
	lowerSize := shared.ControlByteToSize(lower)
	Get4uint32DiffScalar(in, out, lower, prev)
	Get4uint32DiffScalar(in[lowerSize:], out[4:], upper, out[3])
}
...
// Prev: 40
func Get4uint32DiffScalar(in []byte, out []uint32, ctrl uint8, prev uint32) {
